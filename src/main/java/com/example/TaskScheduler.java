...
/**
 * The `TaskScheduler` class manages the scheduling and execution of tasks using
 * a
 * thread pool.
 *
 * The `TaskScheduler` constructor takes the number of threads in the thread
 * pool as
 * a parameter. The `scheduleTask` method is used to schedule a task for
 * execution
 * by the thread pool. The `shutdown` method is used to shut down the task
 * scheduler
 * and the associated thread pool.
 *
 * The `main` method demonstrates the usage of the `TaskScheduler` class by
 * creating
 * a `TaskScheduler` instance with a pool size of 5 threads, scheduling 10 tasks
 * for
 * execution, and then shutting down the scheduler.
 */
